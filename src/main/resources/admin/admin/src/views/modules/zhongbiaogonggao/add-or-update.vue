<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="投标编号" prop="toubiaobianhao">
          <el-select :disabled="ro.toubiaobianhao" @change="toubiaobianhaoChange" v-model="ruleForm.toubiaobianhao" placeholder="请选择投标编号">
            <el-option
                v-for="(item,index) in toubiaobianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="投标价格" prop="toubiaojiage">
          <el-input v-model="ruleForm.toubiaojiage" 
              placeholder="投标价格" clearable  :readonly="ro.toubiaojiage"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="投标价格" prop="toubiaojiage">
              <el-input v-model="ruleForm.toubiaojiage" 
                placeholder="投标价格" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="投标详情" prop="toubiaoxiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.toubiaoxiangqing" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.toubiaoxiangqing" label="投标详情" prop="toubiaoxiangqing">
                    <span v-html="ruleForm.toubiaoxiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.gongyingshangmingcheng = true;
          ...
          if(o=='toubiaojiage'){
            this.ruleForm.toubiaojiage = obj[o];
	    this.ro.toubiaojiage = true;
            continue;
          }
          if(o=='fabushijian'){
	    ...
	    this.ro.fabushijian = true;
          ...
          if(o=='toubiaoxiangqing'){
            this.ruleForm.toubiaoxiangqing = obj[o];
	    this.ro.toubiaoxiangqing = true;
            continue;
          }
        }
      }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/zaixiantoubiao/toubiaobianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.toubiaobianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    toubiaobianhaoChange () {
      this.$http({
        url: `follow/zaixiantoubiao/toubiaobianhao?columnValue=`+ this.ruleForm.toubiaobianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              ...
              if(data.data.toubiaojiage){
                this.ruleForm.toubiaojiage = data.data.toubiaojiage
              }
        } else {
          this.$message.error(data.msg);
        }
	...
	this.ruleForm.toubiaoxiangqing = this.ruleForm.toubiaoxiangqing.replace(reg,'../../../app/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhongbiaogonggao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhongbiaogonggaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
